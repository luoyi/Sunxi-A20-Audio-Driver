...
struct sun4i_dai {
	...
	unsigned int   cur_bclk_div;
	unsigned int   cur_mclk_div;
				  ...
				  unsigned int word_size)
{
	...
	if ( sdai->cur_clk != clk_rate ) {
		printk("change clk rate from %d to %d\n", sdai->cur_clk, clk_rate );
	...
	sdai->cur_bclk_div = bclk_div;
	sdai->cur_mclk_div = mclk_div;
...
MODULE_AUTHOR("Luo Yi <luoyi.ly@gmail.com>");
