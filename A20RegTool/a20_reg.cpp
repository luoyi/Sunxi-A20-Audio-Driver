...
int main(int argc, char ** argv) {
	...
	unsigned char * page = init_mmap(DAC_PAGE, DAC_LEN);
	if ( NULL == page ) {
		return -1;
	}
	uint32_t v = *((uint32_t*)(page + DAC_OFFSET + AC_DAC_DPC_OFFSET));
	std::bitset<32> bs(v);
	...
	dump_value(bs, "PCM", 4, 1);
	dump_value(bs, "Master Slave Select", 5, 1);
	dump_value(bs, "Audio sample select when TX FIFO under run", 6, 1);
	dump_value(bs, "SDO0_EN", 8, 1);
	dump_value(bs, "SDO1_EN", 9, 1);
	dump_value(bs, "SDO2_EN", 10, 1);
	dump_value(bs, "SDO3_EN", 11, 1);

	v = *((uint32_t*)(page + 0x400 + 0x0004));
	bs = v;
	printf("DA0 FMT0: 0x%08X\n", v);
	dump_value(bs, "Serial Data Format", 0, 2);
	dump_value(bs, "Word Select Size", 2, 2);
	dump_value(bs, "Sample Resolution", 4, 2);
	dump_value(bs, "Sample LK Parity", 6, 1);
	dump_value(bs, "Left/ Right Clock Parity", 7, 1);
	v = *((uint32_t*)(page + 0x400 + 0x0008));
	bs = v;
	printf("DA0 FMT1: 0x%08X\n", v);
	v = *((uint32_t*)(page + 0x400 + 0x0014));
	bs = v;
	printf("DA0 DA_FCTL: 0x%08X\n", v);
	dump_value(bs, "RX FIFO Output Mode", 0, 2);
	dump_value(bs, "TX FIFO Output Mode", 2, 1);
	dump_value(bs, "RX FIFO Trigger Level", 4, 6);
	dump_value(bs, "TX FIFO Trigger Level", 12, 7);
	dump_value(bs, "FRX", 24, 1);
	dump_value(bs, "FTX", 25, 1);
	dump_value(bs, "TX FIFO source select", 31, 1);

	v = *((uint32_t*)(page + 0x400 + 0x0018));
	bs = v;
	printf("DA0 DA_FSTA: 0x%08X\n", v);
	dump_value(bs, "RXA_CNT", 0, 7);
	dump_value(bs, "RX FIFO Available", 8, 1);
	dump_value(bs, "TXE_CNT", 16, 8);
	dump_value(bs, "TX FIFO Empty", 28, 1);

	v = *((uint32_t*)(page + 0x400 + 0x001C));
	bs = v;
	printf("DA0 DA_INT: 0x%08X\n", v);
	dump_value(bs, "RXAI_EN", 0, 1);
	dump_value(bs, "RX FIFO Overrun Interrupt Enable", 1, 1);
	dump_value(bs, "RX FIFO Under run Interrupt Enable", 2, 1);
	dump_value(bs, "RX FIFO Data Available DRQ Enable", 3, 1);
	dump_value(bs, "TX FIFO Empty Interrupt Enable", 4, 1);
	dump_value(bs, "TX FIFO Overrun Interrupt Enable", 5, 1);
	dump_value(bs, "TX FIFO Under run Interrupt Enable", 6, 1);
	dump_value(bs, "TX FIFO Empty DRQ Enable", 7, 1);

	v = *((uint32_t*)(page + 0x400 + 0x0024));
	bs = v;
	...
	dump_value(bs, "MCLKO_EN", 7, 1);
	v = *((uint32_t*)(page + 0x400 + 0x0030));
	bs = v;
	printf("DA0 DA_TXCHSEL: 0x%08X\n", v);
	dump_value(bs, "TX Channel Select", 0, 3);
	return 0;
}
